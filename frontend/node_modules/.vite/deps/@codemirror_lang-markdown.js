...
function parseCode(config2) {
  ...
  let wrap = parseMixed((node, input) => {
    ...
    if (codeParser && (id == Type.CodeBlock || id == Type.FencedCode)) {
      ...
      if (parser5)
        return { parser: parser5, overlay: (node2) => node2.type.id == Type.CodeText, bracketed: id == Type.FencedCode };
...
var insertNewlineContinueMarkupCommand = (config2 = {}) => ({ state, dispatch }) => {
  ...
  let dont = null, changes = state.changeByRange((range) => {
    ...
    if (inner.item && emptyLine) {
      ...
      if (first.to >= pos || second && second.to < pos || line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text) || config2.nonTightLists === false) {
