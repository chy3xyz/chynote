...
// Cache unavailability immediately to skip pings in zero_native/chynote-zn dev mode
export function markVaultApiMissing(): void {
  vaultApiAvailable = false;
}

async function checkVaultApi(): Promise<boolean> {
  // Only cache positive results. Negative results are retried on the next
  // call so a vault API server that comes online after first detection
  // failure is picked up automatically.
  if (vaultApiAvailable === true) return true;

  const available = await detectVaultApiAvailability();
  if (available) vaultApiAvailable = true;
  return available;
}
...
function buildPathQueryRequest(
  cmd: PathQueryCommand,
  args: Record<string, unknown>,
): VaultApiRequest | null {
  const payload = commandArgs(args);
  const path = argText(payload, "path");
...
function buildPostRequest(
  ...
  if (cmd === "delete_note")
    return buildRequiredPathPostRequest("delete", payload, {
      path: payload.path,
    });
  return null;
}

function buildVaultApiRequest(
  cmd: string,
  args?: Record<string, unknown>,
): VaultApiRequest | null {
  if (!args) return null;
...
function fetchVaultApiGetRequest(
  request: VaultApiGetRequest,
): Promise<Response> {
...
function fetchVaultApiPostRequest(
  request: VaultApiPostRequest,
): Promise<Response> {
  if (request.kind === "delete")
    return fetch("/api/vault/delete", buildFetchOptions(request));
  if (request.kind === "rename")
    return fetch("/api/vault/rename", buildFetchOptions(request));
  if (request.kind === "rename-filename")
    return fetch("/api/vault/rename-filename", buildFetchOptions(request));
...
export async function tryVaultApi<T>(
  cmd: string,
  args?: Record<string, unknown>,
): Promise<T | undefined> {
  ...
  try {
    ...
    if (cmd === "validate_note_content")
      return (data.content === args?.content) as T;
    return data as T;
  } catch (err) {
    console.warn(
      `[mock-zero] Vault API call failed for ${cmd}, falling back to mock:`,
      err,
    );
    return undefined;
  }
