...
var mac3 = ios || (nav ? /Mac/.test(nav.platform) : false);
...
function captureKeyDown(view, event) {
  ...
  if (code == 8 || mac3 && code == 72 && mods == "c") {
    return stopNativeHorizontalDelete(view, -1) || skipIgnoredNodes(view, -1);
  } else if (code == 46 && !event.shiftKey || mac3 && code == 68 && mods == "c") {
    return stopNativeHorizontalDelete(view, 1) || skipIgnoredNodes(view, 1);
  } else if (code == 13 || code == 27) {
    return true;
  } else if (code == 37 || mac3 && code == 66 && mods == "c") {
    let dir = code == 37 ? findDirection(view, view.state.selection.from) == "ltr" ? -1 : 1 : -1;
    return selectHorizontally(view, dir, mods) || skipIgnoredNodes(view, dir);
  } else if (code == 39 || mac3 && code == 70 && mods == "c") {
    let dir = code == 39 ? findDirection(view, view.state.selection.from) == "ltr" ? 1 : -1 : 1;
    return selectHorizontally(view, dir, mods) || skipIgnoredNodes(view, dir);
  } else if (code == 38 || mac3 && code == 80 && mods == "c") {
    return selectVertically(view, -1, mods) || skipIgnoredNodes(view, -1);
  } else if (code == 40 || mac3 && code == 78 && mods == "c") {
    return safariDownArrowBug(view) || selectVertically(view, 1, mods) || skipIgnoredNodes(view, 1);
  } else if (mods == (mac3 ? "m" : "c") && (code == 66 || code == 73 || code == 89 || code == 90)) {
    return true;
  }
  return false;
}
...
editHandlers.keypress = (view, _event) => {
  let event = _event;
  if (inOrNearComposition(view, event) || !event.charCode || event.ctrlKey && !event.altKey || mac3 && event.metaKey)
    return;
...
var selectNodeModifier = mac3 ? "metaKey" : "ctrlKey";
...
var dragCopyModifier = mac3 ? "altKey" : "ctrlKey";
...
function isNodeViewSelected({
  ...
  if (from2 <= pos && to >= pos + nodeSize2) {
    return true;
  }
  if (selectedOnTextSelection && isTextSelection(selection) && from2 > pos && to < pos + nodeSize2) {
    return true;
  }
  return false;
}
...
function updateMarkViewAttributes(checkMark, editor, attrs = {}) {
  ...
  doc3.descendants((node, pos) => {
    const from2 = tr.mapping.map(pos);
    ...
    if (needsUpdate) {
      ...
      tr.removeMark(from2, to, checkMark.type);
      tr.addMark(from2, to, updatedMark);
    }
  });
  if (tr.docChanged) {
    editor.view.dispatch(tr);
...
//# sourceMappingURL=chunk-UUQDSZ6M.js.map
